...
function PoisonMushroom:updateMush(player)
    hasMush = player:hasCollectible(PoisonMushroom.COLLECTIBLE_POISON_MUSHROOM);
end
...
function PoisonMushroom:onUpdate(player)
    ...
    if Game():GetFrameCount() == 1 then
        Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE, PoisonMushroom.COLLECTIBLE_POISON_MUSHROOM, Vector(320,300), Vector(0,0), nil)   
    end
